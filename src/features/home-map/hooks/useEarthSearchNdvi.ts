...
export function useEarthSearchNdvi({ fieldKey, parcelGeometry, enabled = true }: { fieldKey: string; parcelGeometry?: unknown; enabled?: boolean }) {
  ...
  useEffect(() => {
    ...
    void findLatestSentinel2Scene({ ring, daysBack: 60, maxCloudCover: 30, signal: controller.signal })
      .then(async (scene) => {
  ...
  }, [enabled, fieldKey, parcelGeometry]);
