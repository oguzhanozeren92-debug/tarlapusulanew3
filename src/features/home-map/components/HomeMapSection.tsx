...
export default function HomeMapSection({
  ...
  const displayedSatelliteDate = liveNdvi?.datetime || resolvedHomeSatelliteDate;
      ...
      <div className="tp-field-stats">
        {activeHomeLayer === 'vegetation' ? (
          <>
            ...
            <div><small>Son Görüntü</small><strong>{displayedSatelliteDate ? formatHomeSatelliteDate(displayedSatelliteDate) : earthNdvi.status === 'loading' ? 'Aranıyor' : '—'}</strong>{liveNdvi?.cloudCover != null && <small>Sentinel-2 · Bulut %{Math.round(liveNdvi.cloudCover)}</small>}</div>
          </>
        ) : (
          <>
