...
export function useFieldActivities({
  ...
  const [diagnosisSequenceNo, setDiagnosisSequenceNo] = useState(0);
  ...
  const analyzeDiagnosisPhoto = async (photo: File, sessionId: string | null, sequenceNo: number) => {
    if (!selectedField) {
      setAiAnalysisError('Önce analiz edilecek tarlayı seç.');
      return;
    }
    ...
    try {
      ...
      const compressedPhoto = await compressActivityPhoto(photo);
      let photoRewardHash = `${photo.name}:${photo.size}:${photo.lastModified}`;
      try {
        if (globalThis.crypto?.subtle) {
          const digest = await globalThis.crypto.subtle.digest('SHA-256', await compressedPhoto.arrayBuffer());
          photoRewardHash = Array.from(new Uint8Array(digest)).map((value) => value.toString(16).padStart(2, '0')).join('');
        }
      } catch (hashError) {
      ...
      const { error: uploadError } = await supabase.storage.from('field-activity-photos').upload(uploadedPath, compressedPhoto, {
      ...
      const insertPayload: Record<string, unknown> = {
        ...
        sequence_no: Math.max(1, sequenceNo),
      };
      if (sessionId) insertPayload.diagnosis_session_id = sessionId;

      const { data: job, error: jobError } = await supabase.from('ai_image_analysis_jobs').insert(insertPayload).select('id').single();
      if (jobError) throw jobError;
      jobId = String(job.id);

      const { data, error } = await supabase.functions.invoke('analyze-field-image', { body: { jobId } });
      if (error) throw error;
      if (!data?.analysis) throw new Error(data?.userMessage ?? 'Pusula AI analiz sonucu alınamadı.');

      const result = data.analysis as AiFieldAnalysis & { followUpPhoto?: string; needsMoreEvidence?: boolean };
      const returnedSessionId = typeof data?.diagnosisSessionId === 'string' ? data.diagnosisSessionId : sessionId;
      const returnedSequence = Number(data?.sequenceNo ?? sequenceNo);
      const needsMore = Boolean(data?.needsMoreEvidence ?? result.needsMoreEvidence ?? result.status === 'uncertain');
      const requestedEvidence = String(data?.requestedEvidence ?? result.followUpPhoto ?? '');
      setDiagnosisSessionId(returnedSessionId ?? null);
      setDiagnosisSequenceNo(Number.isFinite(returnedSequence) ? returnedSequence : sequenceNo);
      setDiagnosisNeedsMoreEvidence(needsMore);
      setDiagnosisRequestedEvidence(requestedEvidence);

      let rewardSuffix = '';
      try {
        const reward = await addPoints('PEST_ANALYSIS', {
          dedupeKey: `pest-analysis:${String(selectedField.id)}:${photoRewardHash}`,
          metadata: { source: 'pusula_vision', fieldId: String(selectedField.id), crop: selectedField.crop ?? null, photoHash: photoRewardHash, analysisJobId: jobId, diagnosisSessionId: returnedSessionId },
          toastTitle: 'Pusula AI fotoğraf analizi ödülü',
        });
        if (reward.awarded && reward.awardedPoints > 0) rewardSuffix = ` · +${reward.awardedPoints} Puan`;
      } catch (pointError) {
  ...
  const handleAiAnalyzeActivityPhoto = async () => {
    ...
    await analyzeDiagnosisPhoto(activityPhoto, diagnosisSessionId, diagnosisSessionId ? diagnosisSequenceNo + 1 : 1);
  };

  const handleDiagnosisFollowUpPhoto = async (file?: File) => {
    if (!file || !diagnosisSessionId || !selectedField) return;
    if (!file.type.startsWith('image/')) { setAiAnalysisError('Lütfen bir fotoğraf dosyası seç.'); return; }
    if (file.size > 12 * 1024 * 1024) { setAiAnalysisError('Fotoğraf 12 MB’dan küçük olmalı.'); return; }
    if (activityPhotoPreview.startsWith('blob:')) URL.revokeObjectURL(activityPhotoPreview);
    setActivityPhoto(file);
    setActivityPhotoPreview(URL.createObjectURL(file));
    setAiAnalysisError('');
    await analyzeDiagnosisPhoto(file, diagnosisSessionId, diagnosisSequenceNo + 1);
  };
