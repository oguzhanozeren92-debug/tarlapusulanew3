...
import { getCurrentWeeklyPusulaReport } from '../../features/pusula-pdf/services/pusulaPdfData.service';
import { getPusulaPdfJob, requestPusulaPdf, subscribePusulaPdfJob } from '../../features/pusula-pdf/services/pusulaPdfJob.service';
import { downloadPusulaPdf } from '../../features/pusula-pdf/services/pusulaPdfRenderer.service';
...
export default function AiAnalysisScreen({
  ...
  const handleReadyPusulaPdf = async () => {
    ...
    try {
      ...
      await downloadPusulaPdf(pusulaPdfReport.report_data);
    } catch (error) {
          ...
          <section className="tp-pusula-hub-intro">
            ...
            <div className="tp-pusula-hub-actions">
              ...
              >
                <span>✦</span>
                <strong>
                  {pusulaPdfStatus === 'loading' ? (pusulaPdfJob ? PUSULAPDF_STAGE_LABEL[pusulaPdfJob.stage] : 'PUSULAPDF hazırlanıyor') : pusulaPdfStatus === 'ready' ? 'PUSULAPDF hazır' : 'PUSULAPDF'}
                ...
                <small>
                  {pusulaPdfStatus === 'loading'
                    ...
                    : pusulaPdfStatus === 'ready'
                      ? `${pusulaPdfReport?.report_data?.satellite?.points?.length ?? 0} uydu sahnesi · ${pusulaPdfReport?.report_data?.activities?.length ?? 0} işlem · son 7 günlük snapshot hazır.`
                      : 'Haftalık tarla raporunu gerçek Pusula verileriyle hazırla.'}
                </small>
              </button>
              ...
              >
                ...
                {diagnosisNeedsMoreEvidence ? (
                  ...
                  <>
                    ...
                    >
                      {activityFormLoading ? 'Kaydediliyor...' : '📒 Tarla Geçmişine Kaydet'}
                    </button>
                  </>
                )}
              </div>
